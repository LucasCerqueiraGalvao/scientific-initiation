...
| `keras.ops.nn.dot_product_attention` com backend TensorFlow | Scaled dot-product attention em framework independente | Documentacao Keras Ops + Vaswani et al. (2017) | Reutilizar os mesmos `Q`, `K`, `V` e mascara dos caminhos NumPy/PyTorch em tres casos deterministas. | Passou em `pytest` e na execucao canônica: 9/9 comparacoes aprovadas; maior erro absoluto `2.38418579e-07` para `atol=1e-6` e `rtol=1e-5`. | Aderente | Numerico/algoritmico |
